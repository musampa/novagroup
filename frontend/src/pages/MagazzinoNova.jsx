...
export default function MagazzinoNova() {
  const [magazzinoNova, setMagazzinoNova] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchMagazzinoNova = async () => {
      try {
        const response = await fetch("/api/magazzino/nova");
        if (!response.ok) {
          throw new Error("Errore durante il recupero dei dati del magazzino Nova");
        }
        const data = await response.json();
        console.log("Dati ricevuti da /api/magazzino/nova:", data);
        setMagazzinoNova(data);
      } catch (err) {
  ...
  const tableInstance = useTable({ columns, data: magazzinoNova }, useFilters, useSortBy);
